...
5. **Composite score is 0–90 baseline; 0–100 when the Positioning pillar is active for an F&O-eligible ticker.** The baseline shape is four components: technical 40 + momentum 25 + volume 15 + sentiment 10, cap 90. Every non-F&O ticker (and every F&O ticker with the `NSE_USE_POSITIONING_PILLAR` flag OFF) runs on that shape unchanged. **Ratified exception (2026-09-06, supersedes the 2026-09-03 6a ratification):** F&O-eligible tickers with the flag ON and at least one real positioning input run on **design 6b** from `docs/COMPOSITE_SCORE_SHAPE_REVIEW.md` — the legacy 40+25+15+10=90 shape kept intact, plus positioning as a **pure 10-pt additive overlay**, cap 100. Technical and momentum sub-scores are NOT rescaled; every existing calibration on the 0-90 range stays valid, and grade / action thresholds are unchanged (an F&O ticker with positioning data has a mild additive upgrade path but a non-F&O ticker behaves identically to today). Any further shape change needs the same explicit user ratification, plus capturing a new `data/composite_golden_snapshot.json` and running the [verdict-regression-reviewer](../../agents/verdict-regression-reviewer.md) subagent to explain every ticker delta.
...
- [ ] Composite-score shape unchanged: 4 components 40+25+15+10 cap 90 (non-F&O or Positioning flag OFF), or 4 components + 10 pt Positioning overlay cap 100 (F&O + flag ON + real positioning data, design 6b)
